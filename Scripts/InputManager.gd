...
var _serverSidePlayersInputs : Dictionary = {}
var _localInputDict: Dictionary = {}

func _ready():
	_localInputDict = _createInputDict()

func _createInputDict() -> Dictionary:
	return { 
	"cameraY" : 0.0,
	"inGame_Jump" : false,
	"inGame_Attack1" : false,
	"inGame_Attack2" : false,
...
func _updateInput(inputName, newValue):
	var currentValue = _getInputs(ServerClient._networkId)[inputName]
	if(newValue == currentValue):
		return
	_getInputs(ServerClient._networkId)[inputName] = newValue
	if(ServerClient._networkId > 1):
...
func _unhandled_input(event):
	...
	elif (event.is_action_pressed("inGame_Attack2")):
		_updateInput("inGame_Attack2", true)
	elif (event.is_action_released("inGame_Attack2")):
		_updateInput("inGame_Attack2", false)
